...
The biggest points it gets, in my opinion, is its powerful use of [expression trees](https://learn.microsoft.com/en-us/dotnet/csharp/advanced-topics/expression-trees/). It's only disappointing that this "expression-orientedness" didn't spread to more of the language, since expression-based languages make writing value-oriented (think: immutability-oriented) code a snap. Hence honorable mention.
...
public sealed record NonEmptyString 
{
  ...
  public Result<NonEmptyString, ValidationError> Create(
    string value, 
  ...
  public Result<Option<NonEmptyString>, ValidationError> CreateOption(
    string? value, 
    string fieldName = nameof(NonEmptyString)
  ) => Option
      .Of(value)
