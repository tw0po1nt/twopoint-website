...
export const findSeries = async () => {
  ...
  return posts
    .map(({ series }) => series)
    .filter((s1, idx, arr) => arr.findIndex((s2) => s2?.slug === s1?.slug) === idx)
...
/** */
export const getStaticPathsBlogSeries = async ({ paginate }: { paginate: PaginateFunction }) => {
  if (!isBlogEnabled || !isBlogSeriesRouteEnabled) return [];
  ...
  posts.map((post) => {
    if (post.series?.slug) {
      series[post.series?.slug] = post.series;
    }
  });

  return Array.from(Object.keys(series)).flatMap((seriesSlug) =>
    paginate(
      posts.filter((post) => post.series?.slug && seriesSlug === post.series?.slug),
      {
        params: { series: seriesSlug, blog: SERIES_BASE || undefined },
        pageSize: blogPostsPerPage,
        props: { series: series[seriesSlug] },
      }
    )
  );
};
